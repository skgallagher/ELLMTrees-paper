...
| `fig:layertrace_grid` | `fig3_layertrace_grid.pdf` | ggplot pipeline; fresh Flan profile rescored from `../WeightTraits/outputs/analysis_v20260713/full_finetune/*/model_leaf_analysis/direct_distance_layers.npz` on 46 eligible trees; Llama profile reuses the earlier `scripts/make_layertrace_grid.py` archive | App. B. Key matrices are highlighted against the full per-layer profiles. |
...
| `tab:variants` | controlled training-regime comparison | fresh Flan and Llama rows: `../WeightTraits/paper/weighttraits_variants_registry.yaml` over `outputs/analysis_v20260713/*_summary.json`, `outputs/analysis_llama32_20260722/{r8,r64}/direct_run_set_analysis_summary.json`, and `outputs/analysis_llama32_full_ft_20260722/production_rollup.json` | Flan plus Llama rank-8, rank-64, and full FT use the same 50 topology/task-assignment manifests (**46/26** after recovery/ordering eligibility filters). |
| `tab:layer_subsets` | LayerTrace sparse-layer recovery | rescore the saved `outputs/analysis_v20260713/full_finetune/*/model_leaf_analysis/direct_distance_layers.npz` cubes on the common 46 topology-eligible trees, averaging the named tensor subsets before NJ | enc SA.k: 95.5% clade recovery, 89.1% PAER; identical to full-weight |
