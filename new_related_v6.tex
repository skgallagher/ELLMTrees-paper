...
\textbf{Model weights:} Several works trace weight changes from fine-tuning or RLHF back to parent models. MoTHer \citep{horwitz2025mother} reconstructs directed model trees from weight similarity via a directed minimum spanning tree and evaluates against controlled, known model hierarchies; \citet{yu2025neural} identify parent--child and grandparent--child lineage with both a learning-free fine-tuning approximation and a learned detector ($>99\%$ accuracy). Beyond direct lineage, spectral weight ``fingerprints'' identify common ancestry \citep{ghostspec2025}, and Spectral DeTuning \citep{horwitz2024spectral} reconstructs base weights from shared LoRA variants. Unlike MoTHer, which reconstructs relationships among observed parent and descendant checkpoints, we observe only leaf models and infer a phylogeny with unobserved ancestral branching points. This setting is relevant to LLM family reconstruction, where intermediate or parent checkpoints may not be available.
